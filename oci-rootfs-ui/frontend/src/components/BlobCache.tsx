...
function ExpiryBadge({ expires_at }: { expires_at: string }) {
  if (!expires_at) return <span style={{ fontSize: 11, color: '#bbb' }}>—</span>
  ...
  return (
    <section style={{ marginBottom: '2rem' }}>
      <h2 style={{
        ...
        Blobs — {blobs.length} · {formatBytes(totalRaw)} raw · {formatBytes(totalExtracted)} extrait
      </h2>
        ...
        {blobs.map(blob => (
          <div key={blob.digest} style={{
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
                <span style={{ fontSize: 11, color: '#999' }}>
                  raw <strong style={{ color: '#555' }}>{formatBytes(blob.size)}</strong>
                </span>
                <span style={{ fontSize: 11, color: '#999' }}>
                  extrait <strong style={{ color: '#555' }}>{formatBytes(blob.extracted_size)}</strong>
                </span>
                <span style={{ fontSize: 11, color: '#999' }}>
                  pull <strong style={{ color: '#555' }}>{formatDuration(blob.duration_ms)}</strong>
                </span>
                <span style={{ fontSize: 11, color: '#999' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4 }}>
              ...
              {blob.ttl_hours > 0 && (
                <span style={{ fontSize: 10, color: '#ccc' }}>TTL {blob.ttl_hours}h</span>
              )}
            </div>
          </div>
        ))}
